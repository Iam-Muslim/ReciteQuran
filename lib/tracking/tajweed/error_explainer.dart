...
class ErrorExplainer {
  ...
  static List<PhonemeGroupAlignment> _alignPhonemeGroups(
    ...
    Int32List dp = Int32List((n + 1) * (m + 1));
    int getDp(int i, int j) => dp[i * (m + 1) + j];
    void setDp(int i, int j, int val) => dp[i * (m + 1) + j] = val;

    for (int i = 0; i <= n; i++) setDp(i, 0, i);
    for (int j = 0; j <= m; j++) setDp(0, j, j);

    for (int i = 1; i <= n; i++) {
      for (int j = 1; j <= m; j++) {
        int cost = (refChars[i - 1] == predChars[j - 1]) ? 0 : 1;

        int del = getDp(i - 1, j) + 1;
        int ins = getDp(i, j - 1) + 1;
        int sub = getDp(i - 1, j - 1) + cost;
    ...
    while (i > 0 || j > 0) {
      ...
      if (i > 0 && getDp(i, j) == getDp(i - 1, j) + 1) {
        alignments.add(
          PhonemeGroupAlignment(
            opType: 'delete',
            refIdx: i - 1,
        ...
        i--;
      } else if (j > 0 && getDp(i, j) == getDp(i, j - 1) + 1) {
        alignments.add(
          PhonemeGroupAlignment(
            opType: 'insert',
            refIdx: i, // or -1
  ...
  static Map<int, List<ReciterError>> explainAyahError(
    ...
    int startIdx = targetWordIdx != null ? max(0, targetWordIdx - 1) : 0;
    int endIdx = targetWordIdx != null
        ? targetWordIdx - 1
        : phonemeWords.length - 1;

    for (int wIdx = startIdx; wIdx <= endIdx; wIdx++) {
      ...
      final alignments = _alignPhonemeGroups(refChars, predChars);
