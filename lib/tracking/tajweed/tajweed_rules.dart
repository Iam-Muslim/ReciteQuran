...
abstract class TajweedRule {
  ...
  bool checkDuration(double durationSeconds) {
    return true; // Web doesn't have accurate timestamps from WASM, bypass duration check
  }
...
class AaredMaddRule extends MaddRule {
  ...
  @override
  int count(String refText, String predText) {
    int actualCount = super.count(refText, predText);
    // Madd Aared allows 2, 4, or 6 beats. ASR often outputs just 1 char (2 beats).
    // If we detect at least 1 char, consider the count rule fully satisfied to avoid false errors.
    if (actualCount >= 1) {
      return 100;
    }
    return actualCount;
